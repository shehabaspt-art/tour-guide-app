...
# حالة إظهار أو إخفاء القائمة المخصصة
...
# تقسيم الشاشة لطرفين: القائمة الجانبية المخصصة ومحتوى الصفحة
main_layout_cols = (
    st.columns([1.2, 4.5])
    if st.session_state.sidebar_visible
    else st.columns([0.01, 5])
)

with main_layout_cols[0]:
    if st.session_state.sidebar_visible:
        st.markdown(
            """
            <div style="background-color: #d8ebd8; padding: 15px; border-radius: 10px; border: 2px solid #c2e0c2; min-height: 80vh;">
        ...
        page = st.session_state.get("current_page_saved", "نموذج تصفية المرشد")

with main_layout_cols[1]:
    # زرار السهمين الشيك والبارز للتحكم في الإخفاء والإظهار
    toggle_btn_label = "«« الإخفاء" if st.session_state.sidebar_visible else "»» القائمة"
    if st.button(toggle_btn_label, key="toggle_custom_sidebar"):
        st.session_state.sidebar_visible = not st.session_state.sidebar_visible
        st.session_state.current_page_saved = page
        st.rerun()
